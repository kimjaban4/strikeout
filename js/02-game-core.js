...
function clearReleaseTimingAnimation() {
  if (MP.releaseTimingFrame) {
    window.cancelAnimationFrame(MP.releaseTimingFrame);
...
function updateReleaseTimingCursor() {
  ...
  MP.releaseTimingFrame = window.requestAnimationFrame(updateReleaseTimingCursor);
}

function startReleaseTimingAnimation() {
  if (MP.releaseTimingFrame || !state.releaseTiming?.active) return;
...
function showEventBanner(text, tone = "inning", duration = GAME_TIMING.eventBannerDefault) {
  ...
  MP.inningBannerTimer = window.setTimeout(() => {
    ...
    MP.inningBannerTimer = window.setTimeout(MP.inningBannerDismissHandler, Math.max(0, Number(duration) || GAME_TIMING.eventBannerDefault));
  }, 0);
}
